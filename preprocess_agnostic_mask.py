...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Simple example of Preprocess Agnostic Mask"
    )
    parser.add_argument(
        "--data_root_path",
        type=str,
...
def main(args):
    ...
    for sub_folder in ["upper_body", "lower_body", "dresses"]:
        assert os.path.exists(
            os.path.join(args.data_root_path, sub_folder)
        ), f"Folder {sub_folder} does not exist."
        pair_txt = os.path.join(
            args.data_root_path, sub_folder, "test_pairs_paired.txt"
        )
        ...
        for line in tqdm(lines, desc=f"Processing {sub_folder}"):
            ...
            if os.path.exists(
                os.path.join(output_dir, person_img.replace(".jpg", ".png"))
            ):
                continue
            mask = automasker(
