...
import json
import torch
from model1.attn_processor import AttnProcessor2_0, SkipAttnProcessor
...
def init_diffusion_model(diffusion_model_name_or_path, unet_class=None):
    ...
    text_encoder = CLIPTextModel.from_pretrained(
        diffusion_model_name_or_path, subfolder="text_encoder"
    )
    vae = AutoencoderKL.from_pretrained(diffusion_model_name_or_path, subfolder="vae")
    tokenizer = CLIPTokenizer.from_pretrained(
        diffusion_model_name_or_path, subfolder="tokenizer"
    )
    try:
        ...
        unet.load_state_dict(
            torch.load(
                os.path.join(unet_folder, "diffusion_pytorch_model.bin"),
                map_location="cpu",
            ),
            strict=True,
        )
